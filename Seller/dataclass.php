...
class sellerlogin
{
	...
	public function loginAdmin($email, $password){
		$qA = $this->con->query("SELECT * FROM user_info WHERE email = '$email' and user_type='seller'  LIMIT 1");
        $qB = $this->con->query("SELECT * FROM admin WHERE email = '$email' and seller_or_admin='admin'  LIMIT 1");
		...
		if ($qA->num_rows > 0) {
			...
			if (password_verify($password, $rowA['password'])) {
				$_SESSION['admin_name'] = $rowA['first_name'];
				$_SESSION['admin_id'] = $rowA['user_id'];
